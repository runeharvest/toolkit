...
void NLPACS::CComputableSurface::followBorder(CZoneTessellation *zoneTessel, CSurfElement *first, uint edge, uint sens, vector<CVector> &vstore, bool &loop)
{
	...
	while (true)
	{
		...
		if (((oppositeSurfId != UnaffectedSurfaceId) && (next == NULL || (next->SurfaceId != oppositeSurfId && next->SurfaceId != currentSurfId))) || ((oppositeSurfId == UnaffectedSurfaceId) && ((next != NULL && next->SurfaceId != currentSurfId) || (next == NULL && current->getZoneIdOnEdge(nextEdge) != oppositeZid))) || ((current->EdgeFlag[nextEdge] || (zoneTessel->VerticesFlags[current->Tri[pivot]] != 0)) && !allowThis))
		{
			// if reaches the end of the border, then quits.
			loop = (absoluteEquals(vstore.front(), vstore.back(), 1e-2f) && loopCount != 1);
			break;
		}
		else if ((oppositeSurfId != UnaffectedSurfaceId && next->SurfaceId == oppositeSurfId) || (oppositeSurfId == UnaffectedSurfaceId && next == NULL))
		{
...
void NLPACS::CComputableSurface::buildBorders(CZoneTessellation *zoneTessel)
{
	...
	for (elem = 0; elem < (sint)Elements.size(); ++elem)
	{
		...
		for (edge = 0; edge < 3; ++edge)
		{
			...
			if ((Elements[elem]->EdgeLinks[edge] == NULL || Elements[elem]->EdgeLinks[edge]->SurfaceId != SurfaceId) && !Elements[elem]->EdgeFlag[edge])
			{
	...
	if (ReduceSurfaces)
	{
		...
		for (i = 0; i < 3; ++i)
		{
			for (p = 0; p < (sint)Elements.size(); ++p)
			{
				...
				if (e.IsMergable && &e0 != NULL && &e1 != NULL && &e2 != NULL && e.ZoneId == e0.ZoneId && e.ZoneId == e1.ZoneId && e.ZoneId == e2.ZoneId && !e.ForceInvalid)
				{
					// Strong optimization
		...
		for (p = 0; p < (sint)Elements.size(); ++p)
		{
			...
			if (&e != NULL && &e0 != NULL && &e1 != NULL && &e2 != NULL && e.IsValid && e0.IsValid && e1.IsValid && e2.IsValid && !e.IsUnderWater && e0.IsUnderWater && e1.IsUnderWater && e2.IsUnderWater)
			{
