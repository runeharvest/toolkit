...
LRESULT Browse::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == WM_KEYUP || message == WM_KEYDOWN)
	{
		...
		if (m_ctrl.MousePos.x < (client.right - parent.left) && m_ctrl.MousePos.x > 0 && m_ctrl.MousePos.y < (client.bottom - client.top) && m_ctrl.MousePos.y > 0)
		{
...
unsigned long Browse::MyControllingFunction(void *pParam)
{
	...
	for (int i = 0; i < br->m_ctrl.InfoList.GetSize(br->m_128x128); i++)
	{
		...
		{
		case 0:
			path = tileBank2.getTile(tileBank2.getTileSet(br->m_ctrl.InfoList._tileSet)->getTile128(i))->getRelativeFileName((CTile::TBitmap)(br->m_ctrl.Texture - 1));
			break;
		case 1:
			path = tileBank2.getTile(tileBank2.getTileSet(br->m_ctrl.InfoList._tileSet)->getTile256(i))->getRelativeFileName((CTile::TBitmap)(br->m_ctrl.Texture - 1));
			break;
		case 2: {
...
void Browse::UpdateFlags()
{
	...
	{
		// Set UI
		SubGroup0 = (and&0x1) ? 1 : (or &0x1) ? 2
		                                      : 0;
		SubGroup1 = (and&0x2) ? 1 : (or &0x2) ? 2
		                                      : 0;
		SubGroup2 = (and&0x4) ? 1 : (or &0x4) ? 2
		                                      : 0;
		SubGroup3 = (and&0x8) ? 1 : (or &0x8) ? 2
		                                      : 0;
		SubGroup4 = (and&0x10) ? 1 : (or &0x10) ? 2
		                                        : 0;
		SubGroup5 = (and&0x20) ? 1 : (or &0x20) ? 2
		                                        : 0;
		SubGroup6 = (and&0x40) ? 1 : (or &0x40) ? 2
		                                        : 0;
		SubGroup7 = (and&0x80) ? 1 : (or &0x80) ? 2
		                                        : 0;
		SubGroup8 = (and&0x100) ? 1 : (or &0x100) ? 2
		                                          : 0;
		SubGroup9 = (and&0x200) ? 1 : (or &0x200) ? 2
		                                          : 0;
		SubGroup10 = (and&0x400) ? 1 : (or &0x400) ? 2
		                                           : 0;
		SubGroup11 = (and&0x800) ? 1 : (or &0x800) ? 2
		                                           : 0;
	}
