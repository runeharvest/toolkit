...
CConstraintMeshTexDlg::CConstraintMeshTexDlg(NL3D::CPSConstraintMesh *cm, CWnd *pParent /*=NULL*/)
    : CDialog(CConstraintMeshTexDlg::IDD, pParent)
    , _CurrDlg(NULL)
    , _CM(cm)
{
	nlassert(_CM);
...
void CConstraintMeshTexDlg::setupDlg()
{
	if (_CM->getTexAnimType() == NL3D::CPSConstraintMesh::GlobalAnim)
	{
		...
		((CButton *)GetDlgItem(IDC_REINIT_WHEN_NEW_ELEMENT_IS_CREATED))->SetCheck(_CM->isGlobalAnimTimeResetOnNewElementForced() ? 1 : 0);
	}
	else
	{
