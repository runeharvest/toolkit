...
void getRegisterWindowState(CWnd *pWnd, const TCHAR *keyName, bool resize)
{
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, keyName, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		...
		// Set window pos
		pWnd->SetWindowPos(NULL, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, SWP_NOOWNERZORDER | SWP_NOZORDER | (resize ? 0 : SWP_NOSIZE));
	}
}
...
uint CObjectViewer::addMesh(NL3D::IShape *pMeshShape, const std::string &meshName, uint skelIndex, const char *bindSkelName, bool createInstance)
{
	...
	if (createInstance)
	{
		...
		if (skelIndex != 0xffffffff)
		{
			...
			if ((mesh && mesh->getMeshGeom().isSkinned()) || (meshMrm && meshMrm->getMeshGeom().isSkinned()) || meshMrmSkinned)
			{
				// Bind to skeleton
