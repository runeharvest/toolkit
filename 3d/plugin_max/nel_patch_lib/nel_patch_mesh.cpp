		...
		else
		{
...
#endif // (MAX_RELEASE < 4000)
			{
				...
				if (config == -1)
				{
					...
					int nEdge2;
					int nEdge3;
					if (IsVerticesJoined2(v0, nVert, v2, nEdge0, nEdge1, tab, patch) && IsVerticesJoined2(v1, nVert, v3, nEdge2, nEdge3, tab, patch))
					{
						// additionnal check
						if ((v2 != v3) && (nEdge0 != nEdge1) && (nEdge0 != nEdge2) && (nEdge0 != nEdge3) && (nEdge1 != nEdge2) && (nEdge1 != nEdge3) && (nEdge2 != nEdge3) &&
#if (MAX_RELEASE < 4000)
						    (patch.edges[nEdge0].patch2 == -1) && (patch.edges[nEdge1].patch2 == -1) && (patch.edges[nEdge2].patch2 == -1) && (patch.edges[nEdge3].patch2 == -1) && (!IsVertexInPatch(nVert, patch.edges[nSeg].patch1, patch)) && (!IsVertexInPatch(v2, patch.edges[nSeg].patch1, patch)) && (!IsVertexInPatch(v3, patch.edges[nSeg].patch1, patch)))
#else // (MAX_RELEASE < 4000)
						    (patch.edges[nEdge0].patches.Count() < 2) && (patch.edges[nEdge1].patches.Count() < 2) && (patch.edges[nEdge2].patches.Count() < 2) && (patch.edges[nEdge3].patches.Count() < 2) && (!IsVertexInPatch(nVert, patch.edges[nSeg].patches[0], patch)) && (!IsVertexInPatch(v2, patch.edges[nSeg].patches[0], patch)) && (!IsVertexInPatch(v3, patch.edges[nSeg].patches[0], patch)))
#endif // (MAX_RELEASE < 4000)
						{
							config = 1;
						}
...
bool RPatchMesh::Validity(const PatchMesh &patch, bool bAssert)
{
	...
	for (int nVertex = 0; nVertex < (int)getUIVertexSize(); nVertex++)
	{
		...
		if (getUIVertex(nVertex).Binding.bBinded)
		{
			...
			if ((type != BIND_25) && (type != BIND_50) && (type != BIND_75) && (type != BIND_SINGLE))
			{
				if (bAssert)
		...
		if (getUIVertex(n).Binding.bBinded)
		{
			...
			for (uint nn = 0; nn < listSize; nn++)
			{
				...
				else if (getUIVertex(n).Binding.nType == BIND_50)
				{
					...
					nlassert(nother != -1);
					if ((getUIVertex(nother).Binding.bBinded && (getUIVertex(nother).Binding.nType == BIND_25) && (getUIVertex(nother).Binding.nPrimVert == (uint)n)))
					{
						nlassert(vB == -1);
						vB = pList[nn];
					}
					else if ((getUIVertex(nother).Binding.bBinded && (getUIVertex(nother).Binding.nType == BIND_75) && (getUIVertex(nother).Binding.nPrimVert == (uint)n)))
					{
						nlassert(vA == -1);
						vA = pList[nn];
					}
...
void RPatchMesh::UnbindRelatedVertex(int nVertex, PatchMesh &patch)
{
	...
	{
		if ((getUIVertex(j).Binding.bBinded) && (getUIVertex(j).Binding.nPatch == nFace) && (getUIVertex(j).Binding.nEdge == nEdge))
		{
			UnBindingVertex(j);
		}
	}
										...
										for (int uTile = 0; uTile < nTileCountU; uTile++)
										{
											...
											for (int v = 0; v < nVertexTileCount; v++)
											{
												...
												for (int u = 0; u < nVertexTileCount; u++)
												{
													...
													int nIndex = mpcopy + vTile * nVertexTileCount * nVertexTileCount * nTileCountU + v * nVertexTileCount * nTileCountU + uTile * nVertexTileCount + u;
