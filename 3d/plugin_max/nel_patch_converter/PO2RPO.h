...
public:
	...
	{
		if (localized) return GetString(IDS_CLASS_NAME);
		else return _T("Rykol Patchmesh Converter");
	}
#endif
	...
	{
		if (localized) s = GetString(IDS_CLASS_NAME);
		else s = _T("Rykol Patchmesh Converter");
	}
#endif
	...
	{
		if (localized) return GetString(IDS_PARAMS);
		else return _T("Parameters");
	}
#endif
