...
//	HMODULE hModule = GetModuleHandle("object_viewer_dll_rd.dll");
	...
	if (!hModule)
	{
		::MessageBox(NULL, _T("'hModule' failed at '") __FUNCTION__ _T("' in file '") __FILE__ _T(" on line ") NL_MACRO_TO_STR(__LINE__), _T("NeL Export"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (!res)
	{
		::MessageBox(NULL, _T("'res' failed at '") __FUNCTION__ _T("' in file '") __FILE__ _T(" on line ") NL_MACRO_TO_STR(__LINE__), _T("NeL Export"), MB_OK | MB_ICONERROR);
		return;
	}
