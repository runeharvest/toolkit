...
#endif
{
	...
	{
	case 0:
		if (localized) return TSTR(GetString(IDS_DS_COLOR1));
		else return _T("Color 1");
	case 1:
		if (localized) return TSTR(GetString(IDS_DS_COLOR2));
		else return _T("Color 2");
...
#endif
{
	...
	{
	case 0:
		if (localized) return TSTR(GetString(IDS_DS_PARAMETERS));
