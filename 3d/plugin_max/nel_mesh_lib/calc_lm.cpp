...
bool FaceContinuous(CMesh::CFace *pF1, CMesh::CFace *pF2, bool bTestUV = true, bool bTestNormal = true)
{
	...
	for (j = 0; j < 3; ++j)
		for (i = 0; i < 3; ++i)
		{
			inext = (i == 2) ? 0 : (i + 1);
			jnext = (j == 2) ? 0 : (j + 1);
			if ((pF1->Corner[j].Vertex == pF2->Corner[i].Vertex) && (pF1->Corner[jnext].Vertex == pF2->Corner[inext].Vertex))
			{
				F1c[0] = j;
				F1c[1] = jnext;
				F2c[0] = i;
			...
			if ((pF1->Corner[j].Vertex == pF2->Corner[inext].Vertex) && (pF1->Corner[jnext].Vertex == pF2->Corner[i].Vertex))
			{
				F1c[0] = jnext;
				F1c[1] = j;
				F2c[0] = i;
    ...
    uint32 nLayerNb, CRTWorld &wrt)
{
	...
	for (nPlanes1 = 0; nPlanes1 < nNbPlanes; ++nPlanes1)
	{
		...
		for (i = 0; i < nNbFace1; ++i)
		{
			...
			for (k = nPosMinV; k < nPosMaxV; ++k)
				for (j = nPosMinU; j < nPosMaxU; ++j)
					if ((pP1->msk[j - pP1->x + (k - pP1->y) * pP1->w] == 1) || (pP1->msk[1 + j - pP1->x + (k - pP1->y) * pP1->w] == 1) || (pP1->msk[1 + j - pP1->x + (1 + k - pP1->y) * pP1->w] == 1) || (pP1->msk[j - pP1->x + (1 + k - pP1->y) * pP1->w] == 1))
						if (segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx1, lumy1, lumx2, lumy2) || segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx2, lumy2, lumx3, lumy3) || segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx3, lumy3, lumx1, lumy1) ||
						    ...
						    vertexInSquare(j + 0.5, k + 0.5, 1.0, 1.0, lumx1, lumy1) || vertexInSquare(j + 0.5, k + 0.5, 1.0, 1.0, lumx2, lumy2) || vertexInSquare(j + 0.5, k + 0.5, 1.0, 1.0, lumx3, lumy3))
									...
									for (n = 0; n < pP2->faces.size(); ++n)
									{
										...
										if (FaceContinuous(pF1, pF2))
										{
											for (uint32 o = 0; o < 4; ++o)
											{
												...
												if ((pP1->msk[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w] == 1) && (nAbsX >= pP2->x) && (nAbsX < (pP2->x + (sint32)pP2->w)) && (nAbsY >= pP2->y) && (nAbsY < (pP2->y + (sint32)pP2->h)))
												{
													// Is it an interior calculated pixel ?
													if (pP2->msk[nAbsX - pP2->x + (nAbsY - pP2->y) * pP2->w] == 2)
													{ // Yes -> ok so get it
														pP1->col[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w + pP1->w * pP1->h * nLayerNb] = pP2->col[nAbsX - pP2->x + (nAbsY - pP2->y) * pP2->w + pP2->w * pP2->h * nLayerNb];
														pP1->msk[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w] = 3;
														pP1->ray[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w] = pP2->ray[nAbsX - pP2->x + (nAbsY - pP2->y) * pP2->w];
													}
													else if (pP2->msk[nAbsX - pP2->x + (nAbsY - pP2->y) * pP2->w] == 1)
	...
	for (i = 0; i < AllLights.size(); ++i)
	{
		if (isInteractionLightMesh(AllLights[i], *pMB, *pMBB))
		{
			...
			for (j = 0; j < nNbGroup; ++j)
				if ((AllLights[vvLights[j].operator[](0)].AnimatedLight == AllLights[i].AnimatedLight) && (AllLights[vvLights[j].operator[](0)].LightGroup == AllLights[i].LightGroup))
					break;
...
void convertToWorldCoordinate(CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB, CVector &translation)
{
	...
	for (j = 0; j < pMB->Faces.size(); ++j)
		for (k = 0; k < 3; ++k)
			pMB->Faces[j].Corner[k].Normal = MBMatrix.mulVector(pMB->Faces[j].Corner[k].Normal);
}
