...
void CRTWorld::testCell(CRGBAF &retValue, SGridCell &cell, CVector &vLightPos, CVector &vVertexPos, uint8 &rtVal)
{
	...
	{
		if (cell.pMBB->Materials[cell.pF->MaterialId].getBlend() || cell.pMBB->Materials[cell.pF->MaterialId].getAlphaTest())
		{ // This is a transparent face we have to look in the texture
			...
			{
				retValue = (1.0f - cPixMap.A) * (retValue * (1.0f - cPixMap.A) + retValue * cPixMap * cPixMap.A);
			}
			else
			{
...
void CRTLightAccelPoint::select(CVector &v)
{
	...
	while ((_CurSel < _Selection.size()) && (_Selection[_CurSel].itSel == _Grids[_Selection[_CurSel].nSelGrid].end()))
		++_CurSel;
}
