...
void CExportNel::addBipedNodeTracks(CAnimation &animation, INode &node, const char *parentName, CAnimationBuildCtx *animBuildCtx, bool root, CSSSBuild &ssBuilder)
{
	...
	// Biped node
	if ((transform->ClassID() == BIPSLAVE_CONTROL_CLASS_ID) || (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID))
	{
		// Body biped ?
    ...
    CAnimationBuildCtx *animBuildCtx, bool bodyBiped)
{
	...
	if (ikeys)
	{
		if (ikeys->GetNumKeys() > 0)
		{
			...
			if ((c.ClassID() == Class_ID(HYBRIDINTERP_POSITION_CLASS_ID, 0)) || (c.ClassID() == Class_ID(HYBRIDINTERP_POINT3_CLASS_ID, 0)))
			{
				// For vector
				if ((type == typePos) || (type == typeScale))
			...
			if ((c.ClassID() == Class_ID(TCBINTERP_POSITION_CLASS_ID, 0)) || (c.ClassID() == Class_ID(TCBINTERP_POINT3_CLASS_ID, 0)))
			{
				// For vector
				if ((type == typePos) || (type == typeScale))
...
void CExportNel::buildBipedInformation(CAnimationBuildCtx &animBuildCtx, INode &node)
{
	...
	// Is it a biped node ?
	if (transform && (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID || transform->ClassID() == BIPSLAVE_CONTROL_CLASS_ID))
	{
		// Yes, Add the node
		//===============
...
void CExportNel::overSampleBipedAnimation(CAnimationBuildCtx &animBuildCtx, uint overSampleValue)
{
	...
	for (i = 0; i < animBuildCtx.BipedNodes.size(); i++)
	{
		...
		Control *transform = node->GetTMController();
		nlassert(transform);
		nlassert((transform->ClassID() == BIPSLAVE_CONTROL_CLASS_ID) || (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID));
