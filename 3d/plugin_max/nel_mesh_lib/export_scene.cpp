...
CInstanceGroup *CExportNel::buildInstanceGroup(const vector<INode *> &vectNode, vector<INode *> &resultInstanceNode, TimeValue tvTime)
{
	...
	*/
	...
	for (i = 0; i < (sint)vectNode.size(); ++i, ++it)
	{
		...
		bool bFatherAudible = bAudibleLikeVisible ? bFatherVisible : nAccelType & NEL3D_APPDATA_ACCEL_FATHER_AUDIBLE ? true
		                                                                                                             : false;
		bool bAudibleFromFather = bAudibleLikeVisible ? bVisibleFromFather : nAccelType & NEL3D_APPDATA_ACCEL_AUDIBLE_FROM_FATHER ? true
		                                                                                                                          : false;
					...
					for (j = 0; j < pMB->Faces.size(); ++j)
						if (!facechecked[j])
						{
							...
							for (k = 0; k < 3; ++k)
							{
								...
								{
									if ((pMB->Faces[j].Corner[k].Vertex == poly[m]) && (pMB->Faces[j].Corner[(k + 1) % 3].Vertex == poly[(m + 1) % poly.size()]))
									{
										found = true;
										break;
									}
									if ((pMB->Faces[j].Corner[(k + 1) % 3].Vertex == poly[m]) && (pMB->Faces[j].Corner[k].Vertex == poly[(m + 1) % poly.size()]))
									{
										found = true;
										break;
									}
