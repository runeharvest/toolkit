...
bool SLMPlane::isAllBlack(uint8 nLayerNb)
{
	for (uint32 i = 0; i < w * h; ++i)
		if ((col[i + w * h * nLayerNb].R > 0.06f) || // around 15/255
		    (col[i + w * h * nLayerNb].G > 0.06f) || (col[i + w * h * nLayerNb].B > 0.06f))
...
void SLMPlane::createFromFace(CMesh::CFace *pF)
{
	...
	for (j = decalY; j < (decalY + sizeY - 1); ++j)
		for (k = decalX; k < (decalX + sizeX - 1); ++k)
		{
			...
			else if (segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx1, lumy1, lumx2, lumy2) || segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx2, lumy2, lumx3, lumy3) || segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx3, lumy3, lumx1, lumy1) ||
	...
	for (b = 0; b < this->h; ++b)
		for (a = 0; a < this->w; ++a)
			if (this->msk[a + b * this->w] != 0)
			{
				...
				if ((fabsf(lc.R - color.R) > precision) || (fabsf(lc.G - color.G) > precision) || (fabsf(lc.B - color.B) > precision))
					return false;
			}
	return true;
}
